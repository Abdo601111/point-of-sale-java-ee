...
@Component
@Lazy
public class SalesView extends BaseBean{
	@Autowired
	...
	public void selectCustomer4Invoice(ActionEvent e) {
	...
	public String searchProductAdvanced() {
		try {
			if(selectedProduct.getName().trim().equals("") &&
			   selectedProduct.getCode().trim().equals("") &&
			   selectedProduct.getPricesell() == 0f) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "Enter criteria",  "Enter criteria"));
			}else
				products =  productsService.search4productAdvanced(selectedProduct, priceCriteria, nameCriteria, codeCriteria);
		} catch (Exception e) {
			// TODO: handle exception
		}
		return null;
	...
	private byte priceCriteria, nameCriteria, codeCriteria;
	public byte getPriceCriteria() {
		return priceCriteria;
	}
	public void setPriceCriteria(byte priceCriteria) {
		this.priceCriteria = priceCriteria;
	}
	public byte getNameCriteria() {
		return nameCriteria;
	}
	public void setNameCriteria(byte nameCriteria) {
		this.nameCriteria = nameCriteria;
	}
	public byte getCodeCriteria() {
		return codeCriteria;
	}
	public void setCodeCriteria(byte codeCriteria) {
	...
	public String searchForCustomer() {
		customers = customerService.list(companyId, customerSearchName);
		return null;}
	@Autowired
		...
		try {
			if(invoice.getItems() != null && !invoice.getItems().isEmpty()) {
				for(InvoiceItemDto exIt : invoice.getItems()) {
					if(exIt.getProducId()!=null && exIt.getProducId().equals(item.getProducId()))
							return true;
				}
			}
	...
	public void handleProductSelect(SelectEvent event) {
		String productDescription = event.getObject().toString();
		try {
			for(ProductDto p : products) {
				if(productDescription.toLowerCase().contains(p.getName().toLowerCase())) {
					selectedProduct = p;
					break;
				}
			}
	...
	public List<ProductDto> completeProductByName(String name) {  
		List<ProductDto> suggestions = new ArrayList<>();
		try {
			products = productsService.
					getAvailableProducts(POSConstants.SEARCH_BY_NAME,name, getSelectedBranch());
...
//			        }
	...
	public List<ProductDto> completeProductByCode(String code) {  
		List<ProductDto> suggestions = new ArrayList<>();
		try {
			products = productsService.
					getAvailableProducts(POSConstants.SEARCH_BY_CODE,code, getSelectedBranch());
